...
function LJS_getUrlParams(){let raw_data={};try{raw_data= new URLSearchParams(window.location.search);}catch{return["",""];};let key_data= Array.from(raw_data.keys());if(0<key_data.length){LocalJsStorage_key=key_data[0];LocalJsStorage_args=raw_data.get(key);return[LocalJsStorage_key,LocalJsStorage_args];}else{return["",""];};};
function LJS_makeUrl(key=[],value=""){let skey=key;let svalue=value;if(Array.isArray(key)){skey=key[0];svalue=key[1];};let add="?"+skey+"="+svalue;return window.location.href.split("?")[0]+add;};
...
function LJS_eval(key,args){var funcs=LJS_getMainFuncs();var fun=funcs[key];if(fun !== undefined){LocalJsStorage_key=key;LocalJsStorage_args=args;fun(args);return true;}else{return false;};};
...
function LJS_getJsonChilds(path) {
  let pathF=LJS_formatPath(path);
  let pathFjson=pathF.concat("__json__");
  if (LJS_exists(pathFjson)){
    if (LJS_get(pathFjson)){
      //  path is JSON data
      return [LJS_formatPathStr(pathF)];
    }else{
      // path is a container
      let jsonChilds=[];
      let childsContainer=LJS_childs(path);
      for (let i=0;i<childsContainer.length;i++){
	let contentSubPath=LJS_getJsonChilds(pathF.concat(childsContainer[i]));
	for (let ii=0;ii<contentSubPath.length;ii++){
	  let newValue=contentSubPath[ii];
	  jsonChilds.push(newValue);
	};
      };
      return jsonChilds;
    };
...
console.log(LJS_getJsonChilds("blog"));
console.log(LJS_getJsonChilds("inx"));
