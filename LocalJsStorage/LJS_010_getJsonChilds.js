...
function LJS_getUrlParams(){let raw_data={};try{raw_data= new URLSearchParams(window.location.search);}catch{return["",""];};let key_data= Array.from(raw_data.keys());if(0<key_data.length){let key=key_data[0];let value=raw_data.get(key);return[key,value];}else{return["",""];};};
