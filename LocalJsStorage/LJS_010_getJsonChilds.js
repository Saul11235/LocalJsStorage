...
function LJS_getUrlParams(){let raw_data={};try{raw_data= new URLSearchParams(window.location.search);}catch{return["",""];};let key_data= Array.from(raw_data.keys());if(0<key_data.length){var key=key_data[0];try{LocalJsStorage_key=key;LocalJsStorage_args=value;}catch{};return[key,value];}else{return["",""];};};
