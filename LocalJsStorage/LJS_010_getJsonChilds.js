...
function LJS_getUrlParams(){let raw_data={};try{raw_data= new URLSearchParams(window.location.search);}catch{return["",""];};let key_data= Array.from(raw_data.keys());if(0<key_data.length){let key=key_data[0];try{LocalJsStorage_key=key;LocalJsStorage_args=value;}catch{};return[key,value];}else{return["",""];};};
...
function LJS_eval(key,args){var funcs=LJS_getMainFuncs();var fun=funcs[key];if(fun !== undefined){try{LocalJsStorage_key=key;LocalJsStorage_args=args;}catch{};fun(args);return true;}else{return false;};};
