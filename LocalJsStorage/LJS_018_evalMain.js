...
function LJS_evalMain() {
  let urlParams=LJS_getUrlParams();
  let key=urlParams[0];
  let values=urlParams[1];
  let first=LJS_eval(key,values);
  if (!first) {
    LJS_eval("404",values);
  }
};
...
LJS_setMainFuncs("",myfun);
LJS_setMainFuncs("fun1",myfun1);
