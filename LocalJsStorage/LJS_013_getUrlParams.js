...
function LJS_getUrlParams() {
  let raw_data={};
  try {
  raw_data= new URLSearchParams(window.location.search);
  } catch {
    return ["",""];
  };
  let key_data= Array.from(raw_data.keys());
  if (0<key_data.length){
    // content 
    LocalJsStorage_key=key_data[0];
    LocalJsStorage_args=raw_data.get(key);
    return [LocalJsStorage_key,LocalJsStorage_args];
  }else{
    // void
    return ["",""];
  };
